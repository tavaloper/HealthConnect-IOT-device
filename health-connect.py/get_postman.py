...
def cadastrar_ocorrencia_automaticamente(cpf_usuario_atual : str, usuarios_json : dict, estado_temperatura : float) -> str:
	...
	if usuarios_json[cpf_usuario_atual]['ocorrencias_ativas']:
		return 'Impossivel cadastrar nova ocorrencia enquanto ja existe uma ocorrencia ativa! Marque a ocorrencia atual como Solucionada antes de cadastrar uma nova ocorrencia!'
...
def verifica_tecla_pressionada():
	'''Verifica se usuario apertou tecla 0 para parar monitoramento'''
	try:
		return keyboard.is_pressed('0')
	except:
		return False
...
while True:
	print('\nPRESSIONE 0 REPETIDAS VEZES PARA INTERROMPER LEITURA')
	if verifica_tecla_pressionada():
		print('Tecla pressionada... Interrompendo leitura')
		break
